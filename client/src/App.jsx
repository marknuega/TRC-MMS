...
const emptyFault = () => ({
  ...
  quantity: 1,
  action: "CHANGE",
  company: loadLast().company ?? "PROJECT 2",
  status: "New",
});
...
function nextSeriesNumber(saved, mode, branch) {
  const m =
    String(mode ?? "report").toLowerCase() === "transmittal"
      ? "transmittal"
      : "report";
...
function App({ user, onLogout }) {
  ...
  useEffect(() => {
    if (!isAdmin && NAV.find((n) => n.id === page)?.adminOnly)
      setPage("report");
  }, [isAdmin, page]);
  ...
  function selectBranch(b) {
    ...
    getInventory(isAdmin && b === ALL_BRANCHES ? "" : b)
      .then(setInventory)
      .catch(() => {});
  }
  ...
  async function refreshSaved() {
    try {
      ...
      const sig = reports
        .map((r) => `${r.id}:${r.seq ?? ""}:${r.savedAt ?? r.updatedAt ?? ""}`)
        .join("|");
  ...
  const refreshInventory = () =>
    getInventory(isAllBranches ? "" : branch)
      .then(setInventory)
      .catch(() => {});
  ...
  async function handleLoadReport(rep) {
    if (
      !window.confirm(
        `Load ${repLabel(rep.reportId, rep.branch, rep.mode)} into the form? This replaces the entries currently listed.`,
      )
    )
      return;
    setBusy(true);
  ...
  async function handleDeleteSaved(rep) {
    if (
      !window.confirm(
        `Delete ${repLabel(rep.reportId, rep.branch, rep.mode)}? This cannot be undone.`,
      )
    )
      return;
    try {
      ...
      faults: f.faults.map((fault, idx) => {
        ...
        if (field === "issue") {
          const matched = options.actions.find(
            (a) =>
              a.toUpperCase() === String(e.target.value).trim().toUpperCase(),
          );
    ...
    setEditForm((f) => ({
      ...
      faults: f.faults.map((fault, idx) => {
        ...
        const next = {
          ...fault,
          [field]:
            field === "quantity" ? Number(ev.target.value) : ev.target.value,
        };
        if (field === "issue") {
          const matched = options.actions.find(
            (a) =>
              a.toUpperCase() === String(ev.target.value).trim().toUpperCase(),
          );
          if (matched) next.action = matched;
        }
        return next;
  ...
  const dailySaved = useMemo(
    () => saved.filter((r) => !isTx(r) && inBranch(r)),
    ...
    <>
      <div className="layout no-print">
        <aside className={`sidebar${sidebarCollapsed ? " collapsed" : ""}`}>
          ...
          <div className="side-user">
            <span
              ...
              title={`${user?.username} · ${isAdmin ? "admin" : user?.branch || "user"}`}
            >
              <span className="side-ico">{isAdmin ? "👑" : "👤"}</span>
              <span className="side-label">
                {user?.username}
                <small>
                  {isAdmin ? "Admin · all branches" : user?.branch || "User"}
                </small>
              </span>
            </span>
            <button
        ...
        <main className={`page-main app${WIDE_PAGES.has(page) ? " wide" : ""}`}>
                ...
                <section className="handover">
                  ...
                  {isAllBranches ? (
                    <p className="saved-hint">
                      ...
                      <strong>{reportReceivedBy || "—"}</strong>. Set each
                      branch's names by selecting that branch.
                    </p>
                  ) : (
            ...
            Transmittals move materials and have no CDS code, so it is hidden there. */}
              ...
              <form onSubmit={handleSubmit} className="entry-form">
                ...
                <div className="form-card">
                  ...
                  >
                    <span>
                      {isTransmittal ? "Transmittal Report" : "Faults"}
                    </span>
                      ...
                      <div className="faults">
                        ...
                        {form.faults.map((fault, i) => (
                          ...
                          >
                            ...
                            <input
                              list={
                                isTransmittal ? "materials-list" : "issue-types"
                              }
                              value={fault.issue}
                              onChange={setFault(i, "issue")}
                              placeholder={
                                isTransmittal ? "e.g. A COVER" : "e.g. A COVER"
                              }
              ...
              {!isTransmittal &&
                savedCard({
                  ...
                  placeholder:
                    "🔎 Search inside reports (item, model, branch, date)…",
                })}
              ...
              {isTransmittal &&
                savedCard({
                  ...
                  placeholder:
                    "🔎 Search inside transmittals (item, model, branch, date)…",
                  tx: true,
                })}
            </>
          )}
                    ...
                    <table className="monthly-table">
                      <thead>
                        <tr ref={monthHeadRef}>
                          ...
                          <th rowSpan={2} className="dh col-date">
                            {matrix.rowHeads[0]}
                          </th>
                          <th rowSpan={2} className="dh col-day">
                            {matrix.rowHeads[1]}
                          </th>
          ...
          {page === "inventory" && (
            <Inventory embedded branch={isAllBranches ? "" : branch} />
          )}
