...
const faultIsMeaningful = (f) =>
  f.issue.trim() !== "" || DEVICE_LEVEL.has(String(f.action).toUpperCase());
...
const repLabel = (baseId, branch, mode) => {
  const id =
    mode === "transmittal"
      ? String(baseId ?? "-").replace("REP-", "TRANS-")
      : (baseId ?? "-");
...
const searchById = (list, query) =>
  (list ?? []).filter((r) => docIdMatches(query, docIdsOf(r)));
...
const loadMode = () =>
  lsGet(MODE_KEY) === "transmittal" ? "transmittal" : "report";
...
const loadNumberMode = () =>
  lsGet(NUMBERS_KEY) === "full" ? "full" : "masked";
  ...
  return (
    <div className="multi" ref={ref}>
      <button
        ...
        aria-expanded={open}
      >
        <span className={selected.length ? "" : "multi-ph"}>
      ...
      {open && (
        <div className="multi-menu">
          {options.length === 0 && (
            <div className="multi-empty">No technicians</div>
          )}
          {options.map((opt) => (
...
function App({ user, onLogout }) {
  ...
  const navItems = NAV.filter(
    (n) => !n.adminOnly || isAdmin || (isDirector && n.directorOk),
  );
  ...
  const regionOfBranch = (b) =>
    regionNames.find((r) => (regionMap[r] ?? []).includes(b)) ?? "";
  ...
  useEffect(() => {
    if (isDirector && branch !== ALL_BRANCHES && !branchList.includes(branch))
  ...
  useEffect(() => {
    if (isAdmin && branch !== ALL_BRANCHES && !branchList.includes(branch))
      setBranch(ALL_BRANCHES);
  ...
  useEffect(() => {
    if (
      ...
      adminRegion &&
      regionNames.length &&
      !regionNames.includes(adminRegion)
    )
      setAdminRegion("");
  ...
  useEffect(() => {
    ...
    if (item?.adminOnly && !isAdmin && !(isDirector && item.directorOk))
      setPage("report");
  ...
  const willBeReferenceOnly = useMemo(
    () =>
      (entries ?? []).some((e) =>
        (e.faults ?? []).some((f) => classify(f.action) === "rto"),
      ),
    [entries],
  );
  ...
  const nextSeries = isTransmittal
    ? "TRANS"
    : willBeReferenceOnly
      ? "REF"
      : "REP";
  const autoDocId = isTransmittal
    ? nextTransId
    : willBeReferenceOnly
      ? nextRefId
      : nextReportId;
  ...
  const nextDocId =
    docOverride == null ? autoDocId : `${nextSeries}-${pad4(docOverride)}`;
  ...
  const descByMaterial = useMemo(
    () => materialDescMap(options.materials),
    [options.materials],
  );
  ...
  const issueSuggestions = useMemo(() => {
    ...
    for (const it of options.issueTypes ?? [])
      if (!issueCode(it)) add(issueName(it));
    for (const a of options.actions ?? []) {
      if (!["CHANGE", "REPAIR", "NEW"].includes(String(a).toUpperCase()))
        add(a, "", "action");
    }
  ...
  function removeIssueSuggestion(s) {
    ...
    const where =
      s.source === "action"
        ? "Actions — it will also leave the Action dropdown"
        : "Issue types";
  ...
  function assignIssueCode(name, parts, variant) {
    ...
    const clash = list.find(
      (it) => issueCode(it) === code && sameName(issueName(it), name) === false,
    );
    ...
    const next =
      at >= 0 ? list.map((it, i) => (i === at ? coded : it)) : [...list, coded];
  ...
  useEffect(() => {
    ...
    const onFocus = async () => {
      try {
        const html = await fetch("/", { cache: "no-store" }).then((r) =>
          r.text(),
        );
  ...
  async function refreshSaved() {
    try {
      ...
      const sig = reports
        .map(
          (r) =>
            `${r.id}:${r.seq ?? ""}:${r.savedAt ?? r.updatedAt ?? ""}:${r.isReferenceOnly ? 1 : 0}`,
        )
        .join("|");
  ...
  async function handleLoadReport(rep) {
    if (
      !window.confirm(
        `Load ${shortLabel(rep)} into the form? This replaces the entries currently listed.`,
      )
    )
      return;
    setBusy(true);
  ...
  async function handleToggleReference(rep) {
    ...
    setSavedAll((list) =>
      list.map((r) => (r.id === rep.id ? { ...r, isReferenceOnly: next } : r)),
    );
    try {
      ...
      setSavedAll((list) =>
        list.map((r) =>
          r.id === rep.id ? { ...r, isReferenceOnly: !next } : r,
        ),
      );
      setError(err.message);
    }
  ...
  async function handleDeleteSaved(rep) {
    if (!window.confirm(`Delete ${shortLabel(rep)}? This cannot be undone.`))
      return;
    try {
  ...
  function setCategory(key, list) {
    setOptions((prev) => {
      ...
      saveTimer.current = setTimeout(() => {
        saveOptions(next).catch((err) =>
          setError(`Could not save inputs: ${err.message}`),
        );
      }, 400);
      return next;
    });
  ...
  function agreeWire() {
    ...
    setCategory(
      "agencies",
      withIssiPrefix(options.agencies, wire.agency, wire.prefix),
    );
    setWire(null);
  }
  ...
  const setTel = (e) => {
    ...
    const model = devicePicked.current
      ? ""
      : telPick(telNumber, options.models);
    setForm((f) => ({
      ...f,
      telNumber,
  ...
  const setIssi = (e) => {
    ...
    const agency = agencyPicked.current
      ? ""
      : issiPick(issiNumber, options.agencies);
  ...
  const nextFault = (fault, field, raw) => {
    ...
    if (field === "issue") {
      const matched = options.actions.find(
        (a) => a.toUpperCase() === String(raw).trim().toUpperCase(),
      );
    ...
    const becameNoActivity =
      field === "issue" &&
      isNoActivityIssue(raw) &&
      !isNoActivityIssue(fault.issue);
    ...
    if (next.action !== fault.action) {
      ...
      else if (isServiceAction(fault.action) && !next.company)
        next.company = lastCompany();
    }
  ...
  const setFault = (i, field) => (e) => {
    ...
    setForm((f) => ({
      ...
      faults: f.faults.map((fault, idx) =>
        idx === i ? nextFault(fault, field, e.target.value) : fault,
      ),
    }));
  };
  const addFault = () =>
    setForm((f) => ({ ...f, faults: [...f.faults, emptyFault()] }));
  const removeFault = (i) =>
    setForm((f) => ({
      ...f,
      faults:
  ...
  async function handleSubmit(e, agencyOverride) {
    ...
    try {
      ...
      const payload = {
        ...
        ...(isTransmittal
          ? { type: form.type || "OTHER", model: "", agency: "" }
          : {}),
      ...
      if (payload.faults.length === 0) {
        setError(
          "Add at least one fault — pick an issue, or an action like PROGRAM/INSTALL/DISMANTLE.",
        );
        return;
      }
      ...
      if (!isTransmittal)
        setWire(
          issiWireOffer(payload.issiNumber, payload.agency, options.agencies),
        );
  ...
  const eSet = (field) => (ev) =>
    setEditForm((f) => ({ ...f, [field]: ev.target.value }));
  const eSetModel = (ev) => {
  ...
  const eSetTel = (ev) => {
    ...
    const model = eDevicePicked.current
      ? ""
      : telPick(telNumber, options.models);
    setEditForm((f) => ({
      ...f,
  ...
  const eSetIssi = (ev) => {
    ...
    const agency = eAgencyPicked.current
      ? ""
      : issiPick(issiNumber, options.agencies);
  ...
  const eSetFault = (i, field) => (ev) => {
    ...
    setEditForm((f) => ({
      ...f,
      faults: f.faults.map((fault, idx) =>
        idx === i ? nextFault(fault, field, ev.target.value) : fault,
      ),
    }));
  };
  const eAddFault = () =>
    setEditForm((f) => ({ ...f, faults: [...f.faults, emptyFault()] }));
  ...
  async function handleUpdateEntry(ev) {
    ...
    try {
      const payload = {
        ...
        mode,
        ...(isTransmittal
          ? { type: editForm.type || "OTHER", model: "", agency: "" }
          : {}),
        faults: editForm.faults
          .filter(faultIsMeaningful)
          .map(withSavedQuantity),
      };
      if (payload.faults.length === 0) {
        setError(
          "Add at least one fault — pick an issue, or an action like PROGRAM/INSTALL/DISMANTLE.",
  ...
  async function handleClearAll() {
    if (
      !window.confirm(
        `Clear all ${entries.length} entries? This can't be undone (Save first to keep a copy).`,
      )
    ) {
      return;
    }
  ...
  const reports = useMemo(() => {
    ...
    if (isAllBranches) {
      ...
      const dates = [
        ...new Set(
          entries.map((e) => fmtLongDate(e.reportDate)).filter(Boolean),
        ),
      ];
      const label =
        dates.length <= 1
    ...
    if (dateOverride) {
      return [
        buildDateReport(
          dmyOf(dateOverride),
          repLabel(nextDocId, branch, mode),
          entries,
          opts,
        ),
      ];
  ...
  const combinedTxt = useMemo(
    () => reports.map(buildTxt).join("\n\n\n"),
    [reports],
  );
  ...
  function applyHeaderEdit(e) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
      return setHeaderError("Pick a date for this report");
    const n = parseBlockNumber(typed);
    if (n == null)
      return setHeaderError(
        `"${typed}" is not a document number — try ${blockNumber(autoNumber)}, or ${autoNumber}`,
      );

    // The one the database would refuse, named the way its holder reads it.
    const clash = (saved ?? []).find(
      (r) =>
        seriesOf(r) === nextSeries &&
        String(r.branch ?? "") === String(seriesBranch) &&
        (r.docNumber ?? 0) === n,
    );
    if (clash) {
      return setHeaderError(
        `${shortLabel(clash)} already exists (${clash.dateLabel}) — choose another number`,
      );
    }
  ...
  const { agencyOptions, topAgencies } = useMemo(() => {
    ...
    for (const r of saved ?? [])
      for (const e of r.entries ?? []) bump(e.agency);
    ...
    const byUsage = [...all].sort(
      (a, b) => uses(b) - uses(a) || a.localeCompare(b),
    );
    ...
    const agencyOptions = all.includes(lastAgency)
      ? [lastAgency, ...rest]
      : rest;
  ...
  const sumCols = (cols, source) =>
    cols.reduce((s, c) => s + (source[c.key] || 0), 0);
  ...
  useEffect(() => {
    ...
    Promise.all(
      months.map((mk) => getMonthly(mk, monthBranch).catch(() => null)),
    )
      .then((results) => {
        if (!active) return;
        const byMonth = {};
  ...
  async function handleLoadPaste() {
    ...
    if (Object.keys(data).length === 0) {
      setError(
        "Nothing recognised — paste rows as Date, Day, the 18 columns, then Description (tab-separated).",
      );
      return;
    }
    try {
  ...
  async function handleClearManual() {
    if (
      !window.confirm(
        "Remove the pasted data for this month/branch and revert to live report data?",
      )
    )
      return;
    try {
  ...
  const matrixTitle = () =>
    `Activity ${periodLabel(monthPeriod)}${matrix?.branch ? ` · ${matrix.branch}` : ""}`;
  const matrixSlug = () =>
  ...
  function handleExportMonthlyCsv() {
    ...
    const header = [
      ...matrix.rowHeads,
      ...matrix.columns.map((c) => `${c.group} ${c.label}`),
    ...
    for (const r of matrix.rows) {
      lines.push(
        [
          r.date,
          r.dayName,
          ...matrix.columns.map((c) => r.counts[c.key] || 0),
          r.description,
        ]
          .map(esc)
          .join(","),
    ...
    lines.push(
      ["Total", "", ...matrix.columns.map((c) => matrix.totals[c.key] || 0), ""]
        .map(esc)
        .join(","),
    );
  ...
  function monthlyTableHtml(colgroupHtml = "") {
    ...
    const descHtml = (text) =>
      String(text ?? "")
        ...
        .map((p) =>
          /^\([^)]*[A-Za-z][^)]*\)$/.test(p)
            ? `<span style="color:#c81e1e">${esc(p)}</span>`
            : esc(p),
        )
        .join("");
  ...
  const inBranch = (r) =>
    isAllBranches || String(r.branch ?? "") === String(branch);
  ...
  const dailySaved = useMemo(
    () =>
      saved
        .filter((r) => !isTx(r) && !isRefOnly(r) && inBranch(r))
        .sort(byDocId),
  ...
  const reportResults = useMemo(
    () => searchInside(dailySaved, savedSearch),
    [dailySaved, savedSearch],
  );
  const refResults = useMemo(
    () => searchInside(refSaved, savedRefSearch),
    [refSaved, savedRefSearch],
  );
  const txResults = useMemo(
    () => searchInside(txSaved, savedTxSearch),
    [txSaved, savedTxSearch],
  );
  ...
  const reportIdHits = useMemo(
    () => searchById(dailySaved, savedSearch),
    [dailySaved, savedSearch],
  );
  const refIdHits = useMemo(
    () => searchById(refSaved, savedRefSearch),
    [refSaved, savedRefSearch],
  );
  const txIdHits = useMemo(
    () => searchById(txSaved, savedTxSearch),
    [txSaved, savedTxSearch],
  );
    ...
    <li key={r.id} className={r.isReferenceOnly ? "ref-only" : undefined}>
      <div>
        ...
        {r.isReferenceOnly && (
          <span
            className="ref-badge"
            title="Kept for the record only — no parts were used"
          >
            Reference only
          </span>
        )}{" "}
        <span className="muted small">
          · {r.dateLabel} · {r.entryCount}{" "}
          {r.entryCount === 1 ? "entry" : "entries"} · saved{" "}
      ...
      <ul className="search-results">
        ...
        {results.map((res, idx) => (
          <li key={idx}>
            ...
            <span className="muted small">
              {(tx ? res.receivedBy : res.technician) || "—"}
            </span>
            ...
            <button
              type="button"
              className="res-load"
              onClick={() => handleLoadReport(res.rep)}
              disabled={busy}
            >
              Load
            </button>
  ...
  const savedCard = ({
    ...
    <section className="saved">
      ...
      >
        <span>
          {icon} {title}{" "}
          {list.length > 0 && <span className="hint">({list.length})</span>}
        </span>
        <span className="chev">{open ? "▲" : "▼"}</span>
      </button>
      {open && (
        <div className="saved-body">
          ...
          {list.length === 0 ? (
            ...
            <>
              {idHits.length > 0 && (
                <ul className="saved-list">{idHits.map(savedRow)}</ul>
              )}
              ...
              {(results.length > 0 || idHits.length === 0) &&
    ...
    <>
      <div className="layout no-print">
        ...
        <main className={`page-main app${WIDE_PAGES.has(page) ? " wide" : ""}`}>
                  ...
                  <label className="date-field">
                    ...
                    {lockBranch == null ? (
                      <SearchSelect
                        ...
                        options={[...branchList, ALL_BRANCHES]}
                      />
                    ) : (
            ...
            switch (and Quick Code Entry itself) is hidden there. */}
             ...
             newline. */
                      ...
                      <>
                        {isTransmittal ? (
                          isAllBranches ? (
                            <p className="saved-hint">
                              All Branches: each branch's own handover is added
                              automatically — Transmitted by{" "}
                              <strong>{reportTransmittedBy || "—"}</strong>;
                              Received by{" "}
                              <strong>{reportReceivedBy || "—"}</strong>. Set
                              each branch's names by selecting that branch.
                            </p>
                          ) : (
                        ...
                        <div className="faults">
                          ...
                          {form.faults.map((fault, i) => {
                            ...
                            const locked =
                              !isTransmittal && isNoActivityIssue(fault.issue);
                            return (
                              <div
                              ...
                              >
                                ...
                                <input
                                  ...
                                  min={
                                    isNoActivityIssue(fault.issue) ? "0" : "1"
                                  }
                                  step="1"
                                ...
                                {!isTransmittal && (
                                  <SearchSelect
                                    ...
                                    options={
                                      locked ? [NONE_OPTION] : options.actions
                                    }
                                    ariaLabel="Action"
                                    disabled={locked}
                ...
                is added from that field. */}
                        <datalist id="materials-list">
                          {options.materials.map((m, i) => {
                            ...
                            return (
                              <option key={`mat-${i}-${name}`} value={name} />
                            );
                          })}
                        ...
                        <label className="comment-field">
                          ...
                          <textarea
                            ...
                            placeholder={
                              isTransmittal
                                ? "Note for this transmittal entry…"
                                : "Note for this entry…"
                            }
                          />
                        </label>
                          ...
                          {isTransmittal ? (
                            ...
                            <>
                              ...
                              handset, while protecting nothing that has left. */}
                                {e.technician ? ` · ${e.technician}` : ""} · TEL{" "}
                                {e.telNumber} · ISSI {e.issiNumber}
                              </span>
                              <p>{issueActionCell(e)}</p>
                            </>
                          )}
                          {e.comment && (
                            <p className="entry-comment muted small">
                              💬 {e.comment}
                            </p>
                          )}
                        </div>
                ...
                <div className="modal-backdrop" onClick={closeEdit}>
                  ...
                  >
                    ...
                    <form
                      ...
                      className="entry-form modal-body"
                    >
                      {!isTransmittal && (
                        <div className="grid" onKeyDown={advanceOnEnter}>
                          ...
                          <label>
                            <span className="cap">
                              Technician{" "}
                              <span className="opt">(optional · multiple)</span>
                            </span>
                            <MultiSelect
                              value={editForm.technician}
                              options={technicianNames}
                              onChange={(v) =>
                                setEditForm((f) => ({ ...f, technician: v }))
                              }
                            />
                          </label>
                          <label>
                      ...
                      <div className="faults">
                        <div
                          className={`fault-row fault-head${isTransmittal ? " fault-row--tx" : ""}`}
                        >
                          <span>{isTransmittal ? "Material" : "Issue"}</span>
                          <span>Qty</span>
                          {!isTransmittal && <span>Action</span>}
                        ...
                        {editForm.faults.map((fault, i) => {
                          const locked =
                            !isTransmittal && isNoActivityIssue(fault.issue); // see the entry form's row
                          return (
                            <div
                              className={`fault-row${isTransmittal ? " fault-row--tx" : ""}`}
                              key={i}
                            >
                              {isTransmittal ? (
                                <input
                  ...
                  that cannot be saved at all. */}
                    {reports.length > 0 && (
                      <span className="hint">
                        ...
                        {(dateOverride || docOverride != null) && (
                          <b className="hint-edited"> · edited</b>
                        )}
                        )
                  ...
                  <form className="header-edit" onSubmit={applyHeaderEdit}>
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setHeaderEdit((h) => ({ ...h, date: e.target.value }))
                        }
                      />
                    </label>
                    <label>
                      ...
                      <span className="id-field">
                        ...
                        <input
                          value={headerEdit.id}
                          onChange={(e) =>
                            setHeaderEdit((h) => ({ ...h, id: e.target.value }))
                          }
                    ...
                    {headerError && (
                      <p className="header-edit-error">{headerError}</p>
                    )}
                    <p className="header-edit-note">
                      Applies when you Save. The date moves the entries
                      themselves, so the monthly and spare-parts totals follow
                      it.
                    </p>
                  </form>
                )}
              ...
              select-all copy puts on the clipboard. */}
                <textarea
                  readOnly
                  value={combinedTxt || "No entries yet."}
                  rows={22}
                />
              </section>
              ...
              {!isTransmittal &&
                savedCard({
                  ...
                  empty:
                    "No saved reports yet — in Report mode, click “Save report” above.",
                  placeholder:
                    "🔎 Search reports (id, item, model, branch, date, tel, ISSI)…",
                })}
            ...
            nothing, so it should never be read as one row among the day's work. */}
              {!isTransmittal &&
                savedCard({
                  ...
                  placeholder:
                    "🔎 Search reference-only reports (id, item, model, branch, date, tel, ISSI)…",
                })}
              ...
              {isTransmittal &&
                savedCard({
                  ...
                  empty:
                    "No saved transmittals yet — switch to Transmittal mode and Save.",
                  placeholder:
                    "🔎 Search transmittals (id, item, model, branch, date, tel, ISSI)…",
                  tx: true,
                })}
            </>
          )}
          ...
          {page === "monthly" && (
            <section className="monthly">
              <h2 className="page-title">
                📅 Activity report{" "}
                <span className="hint">· {periodLabel(monthPeriod)}</span>
              </h2>
                ...
                <div className="monthly-body">
                  ...
                  {pasteOpen && (
                    <div className="paste-box">
                      <p className="saved-hint">
                        Paste rows from your sheet (copy from Excel =
                        tab-separated):{" "}
                        <strong>
                          Date, Day, the 18 columns in order, then Description
                        </strong>
                        . Empty cells stay blank. Saved for{" "}
                      ...
                      <textarea
                        ...
                        rows={6}
                        placeholder={
                          "04/01/2026\tSunday\t12\t\t\t\t\t\t\t5\t4\t\t\t\t\t\t\t\t\t(Sepura-Carkit) …"
                        }
                      />
                  ...
                  <p className="saved-hint">
                    {matrix.kind === "month" && manualSheet ? (
                      <>
                        📌 Showing <strong>pasted data</strong> for{" "}
                        {periodLabel(monthPeriod)}
                        {monthBranch ? ` · ${monthBranch}` : ""}. Paste again to
                        replace, or Clear to revert to live.
                      </>
                    ) : (
                      <>
                        Activity counts per terminal, built from your saved{" "}
                        <strong>reports</strong> for {periodLabel(monthPeriod)}.
                        Model cells = total part quantity; Install/Dismantle are
                        per brand.
                        {matrix.kind === "year" &&
                        ...
                        <tr>
                          ...
                          {groupCols.map(({ group, cols }) => {
                            ...
                            return (
                              <th
                                key={group}
                                colSpan={collapsed ? 1 : cols.length}
                                className="grp"
                              >
                                <button
                                  type="button"
                          ...
                          {groupCols.flatMap(({ group, cols }) =>
                            collapsedGroups.has(group)
                              ? [
                                  <th
                                    key={group}
                                    className="col-sub collapsed-col"
                                  >
                                    Σ
                                  </th>,
                                ]
                  ...
                  {matrix.kind === "month" && matrix.rows.length > 7 && (
                    <button
                      type="button"
                      className="month-expand"
                      onClick={() => setMonthExpanded((o) => !o)}
                    >
                      {monthExpanded
                        ? "▲ Show 7 days only"
                        : `▼ View whole month (${matrix.rows.length} days)`}
                    </button>
                  )}
                </div>
              )}
          ...
          {page === "reference" && (
            <ReferenceCard isAdmin={isAdmin} issueTypes={options.issueTypes} />
          )}
...
function PrintDate({ report, descByMaterial, handoverByBranch }) {
  return report.mode === "transmittal" ? (
    <TransmittalPrint
      report={report}
      descByMaterial={descByMaterial}
      handoverByBranch={handoverByBranch}
    />
  ) : (
...
function fmtLongDate(v) {
  ...
  return isNaN(d)
    ? ""
    : `${MON_ABBR[d.getUTCMonth()]} ${d.getUTCDate()}, ${d.getUTCFullYear()}`;
}
...
function TransmittalPrint({
  ...
  const groups = isAll
    ? [
        ...rows.reduce(
          (m, r) =>
            m.set(r.branch || "", [...(m.get(r.branch || "") || []), r]),
          new Map(),
        ),
      ]
        ...
        .map(([b, list]) => [
          b,
          list.slice().sort((x, y) => x.material.localeCompare(y.material)),
        ])
    : null;
            ...
            ? (() => {
                ...
                return groups.flatMap(([b, list]) => {
                  ...
                  const dateSpan = list.map((r, i) => {
                    ...
                    while (
                      i + span < list.length &&
                      fmtLongDate(list[i + span].date) === d
                    )
                      span += 1;
                    return span;
                  });
                  return list.map((r, i) => {
                    ...
                    return (
                      <tr key={`${b}-${i}`}>
                        ...
                        <td>
                          {descByMaterial[String(r.material).toUpperCase()] ||
                            ""}
                        </td>
                        <td>{r.qty}</td>
                        <td>{r.company}</td>
...
function ReportPrint({ report }) {
  ...
  return (
    <section className="print-day">
      ...
      <h4 className="print-split-title">
        Split Format (Airbus / Sepura / Hytera) — Materials Summary
      </h4>
      <SplitColumns byType={materials} />

      <h4 className="print-split-title">
        Split Format (Airbus / Sepura / Hytera) — Device Summary
      </h4>
...
function WireIssiOffer({ wire, onAgree, onDismiss }) {
  ...
  return (
    <p className="wire-issi" role="status" aria-live="polite">
      ...
      <button
        type="button"
        className="agree-pill agree-pill--quiet"
        onClick={onDismiss}
      >
        Not now
      </button>
