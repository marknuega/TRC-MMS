...
const rtoOnly = entry([
  { issue: "SIDE GRIP", quantity: 3, action: "RTO", company: "MOTECO" },
]);
const realWork = entry([
...
describe("RTO stays out of the service totals", () => {
  ...
  test("entryCounts does not grow a stray category key for RTO", () => {
    assert.deepEqual(Object.keys(entryCounts(rtoOnly)).sort(), [
      "dismantle",
      "install",
      "maintenance",
      "programming",
  ...
  test("activityTotals ignores RTO but still counts real work", () => {
    ...
    assert.equal(
      rto.reduce(
        (n, r) => n + r.maintenance + r.program + r.install + r.dismantle,
        0,
      ),
      0,
    );
...
describe("chargers and power supplies count on top of the entry max", () => {
  ...
  test("a charger alongside another part counts as two, not the max", () => {
    assert.equal(
      entryCounts(entry([change("CHARGER12", 1), change("ANTENNA", 1)]))
        .maintenance,
      2,
    );
  });
  ...
  test("the model number may be joined on, spaced, or absent", () => {
    for (const name of [
      ...
      assert.equal(
        entryCounts(entry([change(name, 1), change("ANTENNA", 1)])).maintenance,
        2,
        name,
      );
    }
  ...
  test("a charging pin is an ordinary part, not a standalone item", () => {
    assert.equal(
      entryCounts(entry([change("CHARGING PIN", 1), change("ANTENNA", 1)]))
        .maintenance,
      1,
    );
  });
  ...
  test("each unit counts, so quantity carries", () => {
    assert.equal(
      entryCounts(entry([change("CHARGER12", 10), change("ANTENNA", 1)]))
        .maintenance,
      11,
    );
  });
  ...
  test("the agency summary agrees with it", () => {
    assert.equal(
      agencyComment(makkah),
      ["Agency Summary", DIVIDER, "PSD [MAIN 5]"].join("\n"),
    );
  });
...
describe("the standalone rule reads claimed codes, not names", () => {
  ...
  test("99A is the Charger12", () => {
    claiming(CLAIMS, () =>
      assert.equal(entryCounts(withPart("Charger12")).maintenance, 2),
    );
  });

  test("a 99 part counts standalone however it is named", () => {
    claiming(CLAIMS, () =>
      assert.equal(entryCounts(withPart("DEY-450")).maintenance, 2),
    );
  });
  ...
  test("98 (power supply) is standalone on the same rule", () => {
    claiming(CLAIMS, () =>
      assert.equal(
        entryCounts(withPart("Power Supply - PSE65-12")).maintenance,
        2,
      ),
    );
  });
  ...
  test("a part claimed as something else is not, however charger-ish its name", () => {
    claiming(CLAIMS, () =>
      assert.equal(entryCounts(withPart("CHARGER CABLE")).maintenance, 1),
    );
  });

  test("the claim is matched past case and punctuation", () => {
    claiming(CLAIMS, () =>
      assert.equal(entryCounts(withPart("charger 12")).maintenance, 2),
    );
  });
...
describe("the Materials Summary adds up one line per part", () => {
  ...
  const lines = (entries) =>
    materialBlocksByType(entries).AIRBUS.flatMap((b) => b.lines);
...
describe("the notes block names the device, except where there is none", () => {
  const noted = (model, comment) =>
    entry(
      [{ issue: "ANTENNA", quantity: 1, action: "CHANGE", company: "MOTECO" }],
      { model, comment },
    );
  const txt = (e) =>
    buildTxt(
  ...
  test("an ordinary entry prefixes its note with the model", () => {
    assert.match(
      txt(noted("TH1N", "Bench test passed.")),
  ...
  test("the no-activity record leaves the placeholder model out", () => {
    assert.match(
      txt(noted("For Record Purpose Only.", "Friday, branch closed.")),
  ...
  test("an entry with no model at all does not open with a stray dash", () => {
    assert.match(
      txt(noted("", "Collected from stores.")),
...
describe("short-form document ids", () => {
  ...
  test("the letter blocks hold 999 each, and 000 is never emitted", () => {
    ...
    for (let n = 1; n <= 3000; n++)
      assert.doesNotMatch(blockNumber(n), /000$/, `n=${n}`);
  });
    ...
    for (const [region, branches] of Object.entries(DEFAULT_OPTIONS.regions)) {
      for (const b of branches) {
        assert.equal(
          seen.get(b),
          undefined,
          `${b} is in both ${seen.get(b)} and ${region}`,
        );
        seen.set(b, region);
      }
    }
  ...
  test("the report header carries the short id alone", () => {
    ...
    assert.ok(
      !txt.includes("MAKKAH-REP-0018"),
      "the long form is not printed beside it",
    );
  });
...
describe("finding a document by its id", () => {
  ...
  test("every way someone writes MAKKAH-REP-0018 finds it", () => {
    for (const q of [
      ...
      assert.equal(
        docIdMatches(q, ids18),
        true,
        `"${q}" should find MAKKAH-REP-0018`,
      );
    }
  });
  ...
  test("each series answers to its own short form", () => {
    assert.equal(
      docIdMatches("RTO-A001", ["MAKKAH-REP-0001", "MAK-RTO-A001"]),
      true,
    );
    assert.equal(
      docIdMatches("TRA-A001", ["MAKKAH-TRANS-0001", "MAK-TRA-A001"]),
      true,
    );
    // …and not to another's. A REP and a REF may share a document number.
    assert.equal(
      docIdMatches("RTO-A001", ["MAKKAH-REP-0001", "MAK-REP-A001"]),
      false,
    );
  });
...
describe("Tel / ISSI display", () => {
  ...
  test("the mode rides on the report model, defaulting to complete", () => {
    const opts = { branch: "Makkah" };
    assert.equal(
      buildDateReport("16/08/2026", "MAKKAH-REP-0018", [], opts).numberMode,
      "full",
    );
    assert.equal(
...
describe("a PCB, a programming and an installation on one day", () => {
  ...
  const materialLines = (entries, type) =>
    materialBlocksByType(entries)[type].flatMap((b) => b.lines);
  const agencyOf = (entries, name) =>
    agencyBlocks(entries).find((b) => b.agency === name);
  ...
  describe("the PCB line says which PCB it means", () => {
    ...
    const one = (faults) =>
      materialLines(
        [{ agency: "PRI", type: "AIRBUS", model: "TH1N", faults }],
        "AIRBUS",
      );
    ...
    test("the code does not split the part into two lines", () => {
      assert.deepEqual(
        one([did("PCB", "CHANGE", "MOI"), did("PCB", "REPAIR", "MOTECO")]),
        ["PCB (C) (MOI) = 2"],
      );
    });
    ...
    test("an action outside the code table prints itself, an ordinary part nothing", () => {
      ...
      assert.deepEqual(one([did("ANTENNA", "SOMETHING CUSTOM")]), [
        "ANTENNA (MOI) = 1",
      ]);
    });
  });
  ...
  describe("an action line belongs to the entry that recorded it", () => {
    test("the programming prints under the TH1N and under no other device", () => {
      ...
      assert.ok(
        !materialLines(a011(), "SEPURA").some((l) =>
          l.startsWith("PROGRAMMING"),
        ),
      );
      const labels = (type) =>
        deviceBlocksByType(a011())[type][0].cats.map(([l]) => l);
    ...
    test("the installation prints under the car kit and under no other device", () => {
      ...
      assert.ok(
        !materialLines(a011(), "AIRBUS").some((l) =>
          l.startsWith("INSTALLATION"),
        ),
      );
      assert.ok(
        !deviceBlocksByType(a011()).AIRBUS[0].cats.some(
          ([l]) => l === "INSTALLATION",
        ),
      );
    });
    ...
    test("each lands on the agency of the entry that recorded it", () => {
      ...
      assert.ok(
        !agencyOf(a011(), "PSD").cats.some(([l]) => l === "PROGRAMMING"),
      );
      assert.ok(
        !agencyOf(a011(), "PRI").cats.some(([l]) => l === "INSTALLATION"),
      );
    });
  });
  ...
  describe("Device Summary numbering", () => {
    const deviceSection = (entries) =>
      buildTxt(report(entries))
        .split(`\n${DIVIDER}\nAgency Summary`)[0]
        .split(`Device Summary\n${DIVIDER}\n`)[1];
  ...
  test("the print sheet, the PDF blocks and the WhatsApp text carry the same lines", () => {
    ...
    for (const byType of [
      materialBlocksByType(a011()),
      deviceBlocksByType(a011()),
    ]) {
      for (const type of Object.keys(byType)) {
        for (const b of byType[type]) {
          assert.ok(
            txt.includes(b.header),
            `block "${b.header}" is missing from the text`,
          );
          for (const line of b.lines) {
            ...
            assert.ok(
              txt.includes(body),
              `"${body}" is in the blocks but not in the text`,
            );
          }
        }
      }
