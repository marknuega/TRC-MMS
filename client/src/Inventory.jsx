...
import { CSV_COLUMNS, parsePaste } from './inventoryCsv.js'
...
function downloadCsv(filename, items) {
  ...
  for (const i of items) {
    ...
    // parseDelimited.
    lines.push(CSV_COLUMNS.map(([, read]) => esc(read(i))).join(','))
  }
          ...
          {pasteOpen && (
            <div className="paste-box">
              <p className="saved-hint">
                ...
                . Existing SKUs are updated, new ones added. Those are the positions a paste with NO header row is read
                in — the original sheet's order, unchanged, so an old workbook still pastes correctly. With a header row
                the columns are read by name, in any order, which is how an Export CSV round-trips.
              </p>
              <textarea
                value={pasteText}
